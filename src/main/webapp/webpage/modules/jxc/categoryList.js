...
<script>
		...
		$(document).ready(function() {
			$categoryTreeTable=$('#categoryTreeTable').treeTable({  
		    	   theme:'vsStyle',	           
					...
					column:0,
					checkbox: false,
		            url:'${ctx}/jxc/category/getChildren?parentId=',  
		...
		function del(con,id){  
			jp.confirm('确认要删除类型吗？', function(){
				jp.loading();
	       	  	$.get("${ctx}/jxc/category/delete?id="+id, function(data){
	       	  		if(data.success){
...
<script type="text/html" id="categoryTreeTableTpl">
			<td>
			<c:choose>
			      <c:when test="${fns:hasPermission('jxc:category:edit')}">
				    <a  href="#" onclick="jp.openSaveDialog('编辑类型', '${ctx}/jxc/category/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.name === undefined ? "": d.row.name}}
					</a>
			      </c:when>
			      <c:when test="${fns:hasPermission('jxc:category:view')}">
				    <a  href="#" onclick="jp.openViewDialog('查看类型', '${ctx}/jxc/category/form?id={{d.row.id}}','800px', '500px')">
							{{d.row.name === undefined ? "": d.row.name}}
					</a>
			      </c:when>
			      <c:otherwise>
			...
			<td>
				<div class="btn-group">
				  ...
				  <ul class="dropdown-menu" role="menu">
					<shiro:hasPermission name="jxc:category:view">
						<li><a href="#" onclick="jp.openViewDialog('查看类型', '${ctx}/jxc/category/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-search-plus"></i> 查看</a></li>
					</shiro:hasPermission>
					<shiro:hasPermission name="jxc:category:edit">
						<li><a href="#" onclick="jp.openSaveDialog('修改类型', '${ctx}/jxc/category/form?id={{d.row.id}}','800px', '500px')"><i class="fa fa-edit"></i> 修改</a></li>
		   			</shiro:hasPermission>
		   			<shiro:hasPermission name="jxc:category:del">
		   				<li><a  onclick="return del(this, '{{d.row.id}}')"><i class="fa fa-trash"></i> 删除</a></li>
					</shiro:hasPermission>
		   			<shiro:hasPermission name="jxc:category:add">
						<li><a href="#" onclick="jp.openSaveDialog('添加下级类型', '${ctx}/jxc/category/form?parent.id={{d.row.id}}','800px', '500px')"><i class="fa fa-plus"></i> 添加下级类型</a></li>
					</shiro:hasPermission>
				  </ul>
				</div>
			</td>
