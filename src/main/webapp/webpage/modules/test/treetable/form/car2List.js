...
$(document).ready(function() {
	$('#car2Table').bootstrapTable({
	       ...
	       showSearch: true,
                   ...
                   if($el.data("item") == "edit"){
                       ...
                       view(row.id);
                        ...
                        jp.confirm('确认要删除该车辆记录吗？', function(){
                       	jp.loading();
                       	jp.get("${ctx}/test/treetable/form/car2/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
				   ...
				   <c:choose>
					   <c:when test="${fns:hasPermission('test:treetable:form:car2:edit')}">
					      return "<a href='javascript:edit(\""+row.id+"\")'>"+value+"</a>";
				      </c:when>
					  <c:when test="${fns:hasPermission('test:treetable:form:car2:view')}">
	  ...
	  $('#car2Table').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
            ...
            $('#view,#edit').prop('disabled', $('#car2Table').bootstrapTable('getSelections').length!=1);
        });
		...
		$("#btnImport").click(function(){
			jp.open({
			    ...
			    btn2: function(index, layero){
						...
						iframeWin.contentWindow.importExcel('${ctx}/test/treetable/form/car2/import', function (data) {
							if(data.success){
								jp.success(data.msg);
								refresh();
							}else{
	  ...
	  $("#export").click(function(){//导出Excel文件
			jp.downloadFile('${ctx}/test/treetable/form/car2/export');
	  });
  ...
  function deleteAll(){
		...
		jp.confirm('确认要删除该车辆记录吗？', function(){
			jp.loading();  	
			jp.get("${ctx}/test/treetable/form/car2/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#car2Table').bootstrapTable('refresh');
         	  			jp.success(data.msg);
         	  		}else{
