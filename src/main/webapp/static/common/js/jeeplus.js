...
(function() {
    ...
    jp = {
	   ...
	   confirm:function(msg, succFuc, cancelFuc){//msg:询问信息， succFuc：点‘是’调用的函数， errFuc:点‘否’调用的函数
        ...
        prompt:function (title, value, href) {
            var index = top.layer.prompt({title: title, formType: 0, value: value}, function(text){
	 ...
	 //   },
	   ...
	   /**用户选择框**/
        ...
        dateFormat:function (timestamp, format) {
        	var _this = Object.prototype.toString.call(timestamp).slice(8, -1) === 'Date' ? timestamp : new Date(timestamp);
        	var o = {
               "M+": _this.getMonth() + 1,
               // month
               "d+": _this.getDate(),
	  ...
	  //获取字典标签
		...
		*    beformSubmit：执行提交操作前执行的方法
        ...
        //打开对话框(打印)
        openPrintDialog :function(title,url,width,height, callback){
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
                auto =false;
            }
            top.layer.open({
                ...
                btn: ['打印', '关闭'],
                yes: function(index, layero){
                	var printArea = layero.find('iframe')[0].contentDocument;
                	$(printArea).find("body").jqprint({
